...
function OnWorldPostUpdate()
	...
	if GameGetFrameNum() % 10 == 0 then
		if map then
			...
			-- TODO: Make this entity follow the player or something so it doesn't get frozen?
			...
			if sprite_components then
				for y=0,9 do
					for x=0,9 do
						...
						if biome_x == 36 and biome_y == 11 then
							print(chunk.r)
							print(chunk.g)
							print(chunk.b)
						end
						-- print(bla)
						ComponentSetValue2(sprite_components[(x+1)+(y*10)], "rect_animation", bla)
					end
				end
			end
...
function OnMagicNumbersAndWorldSeedInitialized()
	...
	-- local xml2lua = dofile_once("mods/LocationTracker/lib/xml2lua.lua")
	-- xml2lua.parse()
...
function OnPlayerSpawned(player)
	...
	EntityLoad("mods/LocationTracker/yo.xml", 50, 50)
end
